...
"""
Code borrowed from: https://github.com/OldDriver-Lab/my-probabilistic-suffix-trees/blob/master/SuffixTree.py
"""
...
class Tree(object):
    ...
    def get_predictive_distribution(self, context):
        node = self.root
        depth = 1
        while (context[-depth:] in node.children) and (depth < self.L) and (depth <= len(context)):
            node = node.children[context[-depth:]]
            depth += 1
        return node.probability_vector

    def get_probability(self, context, next_element):
        node = self.root
        depth = 1
        while (context[-depth:] in node.children) and (depth < self.L) and (depth <= len(context)):
            # print(context[-depth:])
            node = node.children[context[-depth:]]
            depth += 1
        return node.probability_vector[next_element]
    ...
    def predict(self, X):
        ...
        for i_segment in range(len(X)):
            ...
            X_segment = X[i_segment]

            # For the PST, we need to replace the start and stop tokens with '<' and '>'
            X_segment = ['<' if x == 'START' else '>' if x == 'STOP' else x for x in X_segment]
            X_segment = ''.join(X_segment)

            for t in range(len(X_segment)):

                context, x = X_segment[max(0, t - self.L):t], X_segment[t]
                likelihoods.append(self.get_probability(context, x))
